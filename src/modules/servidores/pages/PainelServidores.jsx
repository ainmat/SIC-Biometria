...
const CORES_SEC   = ['#6366f1','#3b82f6','#10b981','#f97316','#ec4899','#a855f7','#14b8a6','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#84cc16','#f43f5e','#0ea5e9','#d946ef'];
const CORES_PIE   = ['#6366f1','#10b981','#f97316','#ec4899','#a855f7','#3b82f6','#f59e0b','#14b8a6'];
const CORES_FAIXA = ['#c7d2fe','#a5b4fc','#818cf8','#6366f1','#4f46e5','#4338ca','#3730a3','#312e81','#94a3b8'];

function CustomTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background: 'rgba(10,17,32,.97)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 10, padding: '8px 14px', fontSize: 12, boxShadow: '0 8px 32px rgba(0,0,0,.6)' }}>
...
function KpiCard({ icon: Icon, label, value, sub, cor = '#6366f1' }) {
  return (
    <div style={{ flex: 1, minWidth: 0, background: 'linear-gradient(160deg, rgba(255,255,255,.04) 0%, rgba(255,255,255,.015) 100%)', border: '1px solid rgba(255,255,255,.08)', borderTop: `3px solid ${cor}`, borderRadius: 14, padding: '18px 20px' }}>
      ...
      <div style={{ fontSize: 36, fontWeight: 800, color: '#f8fafc', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{fmt(value)}</div>
...
function MiniPieCard({ title, data, total }) {
  return (
    <div style={{ flex: 1, minWidth: 0, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12, padding: '16px 18px' }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.06em' }}>{title}</div>
        ...
        {data.slice(0, 5).map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 10, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</span>
            </div>
            <span style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', flexShrink: 0 }}>{pct(d.value, total)}%</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="content">
        ...
        {!loading && !erro && total === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <div style={{ fontSize: 15, color: '#475569', fontWeight: 600 }}>Nenhum servidor encontrado</div>
          </div>
        )}
        ...
        {!loading && total > 0 && (
          <>
            ...
            <div style={{ padding: '10px 14px', borderRadius: 8, background: 'rgba(245,158,11,.07)', border: '1px solid rgba(245,158,11,.2)', color: '#fbbf24', fontSize: 11, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {[
                ...
                return (
                  <Link key={label} to={link} style={{ flex: 1, minWidth: 155, textDecoration: 'none' }}>
                    <div style={{ padding: '14px 16px', borderRadius: 12, background: 'linear-gradient(160deg, rgba(255,255,255,.04) 0%, rgba(255,255,255,.015) 100%)', border: `1px solid ${status === 'alert' ? 'rgba(239,68,68,.3)' : status === 'warn' ? 'rgba(245,158,11,.3)' : 'rgba(255,255,255,.08)'}`, borderTop: `3px solid ${cor}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <Icon size={14} color={cor} />
                        <span style={{ fontSize: 10, color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.07em' }}>{label}</span>
                        <span style={{ width: 7, height: 7, borderRadius: '50%', background: cor, marginLeft: 'auto' }} />
                      </div>
                      <div style={{ fontSize: 26, fontWeight: 800, color: '#f8fafc', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    {saudeRisco.map(({ sec, pctC }) => (
                      <span key={sec} style={{ fontSize: 11, color: '#94a3b8' }}>
                        <b style={{ color: '#f87171' }}>{sec}</b> ({pctC.toFixed(0)}%)
                      </span>
                    ))}
                  </div>
                </div>
                <Link to="/servidores/saude" style={{ textDecoration: 'none', color: '#f87171', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>
                  Ver Índice de Saúde →
                </Link>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
              <KpiCard icon={Users}      label="Total de Servidores" value={total}            sub={`${totalSecs} secretaria${totalSecs !== 1 ? 's' : ''}`} cor="#6366f1" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 240px', gap: 14, marginBottom: 20, alignItems: 'start' }}>
              <div className="chart-card" style={{ minWidth: 0 }}>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#f1f5f9' }}>Servidores por secretaria</div>
                  <div style={{ fontSize: 12, color: '#475569' }}>Top {bySecretaria.length}</div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
              <div className="chart-card">
                <div style={{ fontSize: 14, fontWeight: 700, color: '#f1f5f9', marginBottom: 14 }}>Nível de escolaridade</div>
                <ResponsiveContainer width="100%" height={Math.max(byEscol.length * 32 + 20, 80)}>
                  <BarChart data={byEscol} layout="vertical" margin={{ left: 8, right: 50, top: 0, bottom: 0 }}>
                    ...
                    <YAxis type="category" dataKey="name" width={200} tick={{ fill: '#64748b', fontSize: 11 }} tickLine={false} axisLine={false} />
                    <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,.04)' }} />
                    <Bar dataKey="value" radius={[0, 6, 6, 0]} maxBarSize={20} fill="#8b5cf6" label={{ position: 'right', fill: '#475569', fontSize: 11, formatter: fmt }} />
                  </BarChart>
                </ResponsiveContainer>
              </div>

              <div className="chart-card">
                <div style={{ fontSize: 14, fontWeight: 700, color: '#f1f5f9', marginBottom: 14 }}>Distribuição por faixa etária</div>
            ...
            {!filtroSec && (
              <div className="chart-card">
                <div style={{ fontSize: 14, fontWeight: 700, color: '#f1f5f9', marginBottom: 16 }}>Resumo por secretaria</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr>
                        {['Secretaria','Sigla','Servidores','% do total'].map(h => (
                          <th key={h} style={{ padding: '8px 12px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: '#475569', textAlign: h === 'Servidores' || h === '% do total' ? 'right' : 'left', borderBottom: '1px solid rgba(255,255,255,.06)', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {agrupar(dados, 'Des_Secretaria').map((row, i) => {
                        ...
                        return (
                          <tr key={row.name} style={{ borderBottom: '1px solid rgba(255,255,255,.03)' }}>
                            <td style={{ padding: '9px 12px', fontSize: 12, color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: 8 }}>
                              <span style={{ width: 8, height: 8, borderRadius: 2, background: CORES_SEC[i % CORES_SEC.length], flexShrink: 0 }} />
                              {row.name}
                            </td>
                            <td style={{ padding: '9px 12px', fontSize: 11, color: '#a5b4fc', fontFamily: 'monospace', fontWeight: 700 }}>{sigla}</td>
                            <td style={{ padding: '9px 12px', fontSize: 12, fontWeight: 700, color: '#f1f5f9', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(row.value)}</td>
                            <td style={{ padding: '9px 12px', fontSize: 12, color: '#64748b', textAlign: 'right' }}>{pct(row.value, dados.length)}%</td>
                          </tr>
                        );
                      })}
                    </tbody>
