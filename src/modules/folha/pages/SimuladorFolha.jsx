  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
      <div style={{
        ...
        background: done ? '#3b82f6' : 'rgba(255,255,255,.08)',
        border: `1px solid ${done ? '#3b82f6' : 'rgba(255,255,255,.12)'}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 10, fontWeight: 700, color: done ? '#fff' : '#64748b', flexShrink: 0,
      }}>{n}</div>
      <span style={{ fontSize: 11, color: done ? '#93c5fd' : '#64748b', textTransform: 'uppercase', letterSpacing: '.06em', fontWeight: 600 }}>
        {label}
      </span>
    </div>
  );
...
function HistTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: 'rgba(15,23,42,.96)', border: '1px solid rgba(99,102,241,.3)', borderRadius: 8, padding: '10px 14px', fontSize: 11 }}>
      <div style={{ fontWeight: 700, color: '#a5b4fc', marginBottom: 6 }}>{label}</div>
      <div style={{ color: '#f87171' }}>Faltas: <strong>{d?.faltas ?? 0}</strong></div>
      ...
      <div style={{ color: '#94a3b8', marginTop: 4, borderTop: '1px solid rgba(255,255,255,.06)', paddingTop: 4 }}>
        Total desconto: <strong style={{ color: '#f1f5f9' }}>{(d?.totalDesconto ?? 0).toFixed(3).replace('.', ',')}</strong>
      </div>
    </div>
  );
}
...
export default function SimuladorFolha() {
  ...
  const selStyle = { width: '100%', borderRadius: 7, border: '1px solid rgba(255,255,255,.1)', color: '#f1f5f9', fontSize: 13 };
        ...
        <div className="sim-grid" style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 16, alignItems: 'start' }}>
          ...
          <div className="chart-card no-print">
            ...
            <div style={{ marginLeft: 30 }}>
              ...
              />
              <div style={{ maxHeight: 220, overflowY: 'auto', borderRadius: 6, border: '1px solid rgba(255,255,255,.07)' }}>
                ...
                {servidoresFiltrados.map(s => (
                  <div
                    ...
                    style={{
                      ...
                      background: selMat === String(s.matricula) ? 'rgba(59,130,246,.12)' : 'transparent',
                      borderBottom: '1px solid rgba(255,255,255,.04)', transition: 'background .1s',
                    }}
                    onMouseOver={e => { if (selMat !== String(s.matricula)) e.currentTarget.style.background = 'rgba(255,255,255,.04)'; }}
                    onMouseOut={e => { if (selMat !== String(s.matricula)) e.currentTarget.style.background = 'transparent'; }}
                  >
                    <div style={{ fontSize: 12, color: '#f1f5f9', fontWeight: 500 }}>{s.nome || `Matrícula ${s.matricula}`}</div>
          ...
          <div>
            {!servidor && (
              <div className="chart-card" style={{ textAlign: 'center', padding: '60px 40px', color: '#475569' }}>
              ...
              <>
                ...
                <div className="chart-card" style={{ marginBottom: 14 }}>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 16, fontWeight: 700, color: '#f1f5f9' }}>{servidor.nome || '—'}</div>
                      <div style={{ fontSize: 12, color: '#64748b', marginTop: 3 }}>
                        Mat. <span style={{ color: '#60a5fa', fontFamily: 'monospace', fontWeight: 600 }}>{servidor.matricula}</span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                      <div style={{ display: 'flex', gap: 4, padding: '3px', borderRadius: 8, background: 'rgba(255,255,255,.05)' }}>
                        <button
                          onClick={() => setAbaHist(false)}
                          style={{
                            padding: '4px 12px', borderRadius: 6, border: 'none', cursor: 'pointer',
                            fontSize: 11, fontWeight: 600,
                            background: !abaHist ? 'rgba(99,102,241,.25)' : 'transparent',
                            color: !abaHist ? '#a5b4fc' : '#64748b', transition: 'all .15s',
                          }}
                        >Verbas</button>
                        <button
                        ...
                        >
                          Histórico
                          {historico.length > 0 && (
                            <span style={{ marginLeft: 5, fontSize: 9, background: 'rgba(99,102,241,.3)', color: '#c7d2fe', padding: '1px 5px', borderRadius: 9 }}>
                              {historico.length}
                            </span>
                          )}
                        </button>
                ...
                {!abaHist && (
                  <>
                        ...
                        {['detalhado', 'resumido'].map(v => (
                          <button
                            ...
                            style={{
                              ...
                              color: vista === v ? '#a5b4fc' : '#64748b', transition: 'all .15s',
                            }}
                          >{v}</button>
                        ))}
                      </div>
                    ...
                    {vista === 'detalhado' && (
                      <>
                        <div className="chart-card" style={{ marginBottom: 14 }}>
                          <div className="chart-title" style={{ marginBottom: 14, color: '#f87171' }}>Descontos</div>
                        ...
                        <div className="chart-card" style={{ marginBottom: 14 }}>
                          <div className="chart-title" style={{ marginBottom: 14, color: '#34d399' }}>Horas Extras e Adicionais</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            <NumBadge valor={he50}        label="Hora extra 50%"    cor="#10b981" />
                            <NumBadge valor={he100}       label="Hora extra 100%"   cor="#10b981" />
                            <NumBadge valor={adicNoturno} label="Adicional noturno" cor="#8b5cf6" />
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            ...
                            {(atrasosDia > 0 || atrasosFracao > 0) && (
                              <div style={{ padding: '12px 16px', borderRadius: 8, background: 'rgba(239,68,68,.06)', border: '1px solid rgba(239,68,68,.18)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                  <div style={{ fontSize: 13, color: '#94a3b8' }}>Total Atrasos — dias de desconto</div>
                                ...
                                <span style={{ fontSize: 16, fontWeight: 800, color: '#f87171', fontFamily: 'monospace' }}>
                            ...
                            <NumBadge valor={dsr} label="DSR — domingos perdidos" cor="#f97316" negativo />
                            {faltas === 0 && atrasosDia === 0 && atrasosFracao === 0 && dsr === 0 && (
                              <div style={{ fontSize: 13, color: '#64748b', textAlign: 'center', padding: '8px 0' }}>Nenhum desconto nesta competência</div>
                            )}
                          </div>
                        </div>
                        <div className="chart-card" style={{ marginBottom: 14 }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {(he50 > 0 || he100 > 0) && (
                              <div style={{ padding: '12px 16px', borderRadius: 8, background: 'rgba(16,185,129,.06)', border: '1px solid rgba(16,185,129,.18)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                  <div style={{ fontSize: 13, color: '#94a3b8' }}>Total Hora Extra</div>
                                  <div style={{ fontSize: 10, color: '#475569', marginTop: 3 }}>
                                ...
                                <span style={{ fontSize: 16, fontWeight: 800, color: '#34d399', fontFamily: 'monospace' }}>{totalHE}h</span>
                              </div>
                            )}
                            <NumBadge valor={adicNoturno} label="Adicional noturno" cor="#8b5cf6" />
                            {he50 === 0 && he100 === 0 && adicNoturno === 0 && (
                              <div style={{ fontSize: 13, color: '#64748b', textAlign: 'center', padding: '8px 0' }}>Nenhuma hora extra nesta competência</div>
                            )}
                          </div>
                        </div>
                      </>
                    ...
                    <div className="chart-card" style={{ borderLeft: '3px solid #6366f1' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div style={{ padding: '12px 16px', borderRadius: 8, background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.15)', textAlign: 'center' }}>
                          <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.05em' }}>Total dias descontados</div>
                          <div style={{ fontSize: 24, fontWeight: 800, color: '#f87171', fontFamily: 'monospace' }}>
                        ...
                        <div style={{ padding: '12px 16px', borderRadius: 8, background: 'rgba(16,185,129,.08)', border: '1px solid rgba(16,185,129,.15)', textAlign: 'center' }}>
                          <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.05em' }}>Total horas extras</div>
                          <div style={{ fontSize: 24, fontWeight: 800, color: '#34d399', fontFamily: 'monospace' }}>{totalHE}h</div>
                        </div>
                      </div>
                    </div>
                  </>
                ...
                {abaHist && (
                  <>
                    {historico.length === 0 ? (
                      <div className="chart-card" style={{ textAlign: 'center', padding: '40px 24px', color: '#475569' }}>
                      ...
                      <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 14 }}>
                          <div className="kpi-card">
                            <div className="kpi-accent" style={{ background: '#6366f1' }} />
                            <div className="kpi-label">Competências</div>
                            <div className="kpi-value" style={{ color: '#a5b4fc', fontSize: 26 }}>{historico.length}</div>
                          </div>
                          <div className="kpi-card" style={{ borderColor: consecutivos >= 3 ? 'rgba(239,68,68,.25)' : undefined }}>
                            ...
                            <div className="kpi-value" style={{ color: consecutivos >= 3 ? '#f87171' : consecutivos >= 1 ? '#fbbf24' : '#34d399', fontSize: 26 }}>
                              {consecutivos}
                            </div>
                          </div>
                          <div className="kpi-card">
                            ...
                            <div className="kpi-value" style={{ color: '#f87171', fontSize: 26 }}>
                          ...
                          <div style={{ height: 220 }}>
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={chartData} margin={{ top: 4, right: 8, left: -20, bottom: 4 }}>
                                ...
                                <Tooltip content={<HistTooltip />} cursor={{ fill: 'rgba(99,102,241,.08)' }} />
                            ...
                            <table>
                              ...
                              <tbody>
                                {[...historico].sort((a, b) => b.competencia.localeCompare(a.competencia)).map(h => {
                                  ...
                                  return (
                                    <tr key={h.competencia} style={{ background: h.competencia === selComp ? 'rgba(99,102,241,.08)' : undefined }}>
                                      <td style={{ fontFamily: 'monospace', fontSize: 11, color: h.competencia === selComp ? '#a5b4fc' : '#94a3b8', fontWeight: h.competencia === selComp ? 700 : 400 }}>
                                        {fmtCompetencia(h.competencia)}
                                        {h.competencia === selComp && <span style={{ marginLeft: 6, fontSize: 9, color: '#6366f1' }}>← atual</span>}
                                      </td>
                                      <td style={{ textAlign: 'center', fontFamily: 'monospace', fontSize: 11, color: h.faltas > 0 ? '#f87171' : '#374151' }}>{h.faltas}</td>
                                      <td style={{ textAlign: 'center', fontFamily: 'monospace', fontSize: 11, color: h.atrasos_fracao > 0 ? '#fbbf24' : '#374151' }}>{h.atrasos_fracao}</td>
                                      ...
                                      <td style={{ textAlign: 'right', fontFamily: 'monospace', fontSize: 11, fontWeight: 700, color: temDesc ? '#f87171' : '#374151' }}>
