          ...
          {porSecretaria.map(s => (
            <Fragment key={s.sigla}>
              <tr
                ...
                style={{ borderBottom: '1px solid rgba(255,255,255,.03)', cursor: s.unidades.length > 0 ? 'pointer' : 'default' }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,.02)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <td style={{ padding: '7px 10px', color: '#a5b4fc', fontWeight: 700 }}>{s.sigla}</td>
                <td style={{ padding: '7px 10px', color: '#f1f5f9' }}>{s.nome}</td>
                <td style={{ padding: '7px 10px', color: '#10b981', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{fmt(s.servidores)}</td>
                <td style={{ padding: '7px 10px', color: '#3b82f6', fontVariantNumeric: 'tabular-nums' }}>{fmt(s.marcacoes)}</td>
              ...
              {expandidas.has(s.sigla) && s.unidades.map(u => (
                <tr key={u.nome} style={{ background: 'rgba(99,102,241,.03)', borderBottom: '1px solid rgba(255,255,255,.02)' }}>
                  <td style={{ padding: '5px 10px 5px 22px', color: '#475569', fontSize: 10 }}>↳</td>
                  <td style={{ padding: '5px 10px', color: '#94a3b8', fontSize: 11 }}>{u.nome}</td>
                  <td style={{ padding: '5px 10px', color: '#10b981', fontSize: 11, fontVariantNumeric: 'tabular-nums' }}>{fmt(u.servidores)}</td>
                  <td style={{ padding: '5px 10px', color: '#3b82f6', fontSize: 11, fontVariantNumeric: 'tabular-nums' }}>{fmt(u.marcacoes)}</td>
                  <td />
                </tr>
              ))}
            </Fragment>
        ...
        <div style={CARD}>
          ...
          >
            ...
            <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 4 }}>
              {arquivo
                ? <><b style={{ color: '#f1f5f9' }}>{arquivo.name}</b> ({(arquivo.size / 1024).toFixed(0)} KB)</>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            {compDetec && (
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 16, lineHeight: 1.8 }}>
                <div>Arquivo detectado: <b style={{ color: '#94a3b8' }}>{compLabel(compDetec)}</b></div>
                <div>Competência da folha: <b style={{ color: '#a5b4fc' }}>{compLabel(competencia)}</b></div>
              </div>
            )}
          </div>
        ...
        <>
            ...
            {join.orfasArq.length > 0 && (
              <div style={{ background: 'rgba(245,158,11,.05)', border: '1px solid rgba(245,158,11,.15)', borderRadius: 8, padding: '10px 14px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  ...
                  {join.orfasArq.length > 80 && <span style={{ fontSize: 10, color: '#475569' }}>…e mais {fmt(join.orfasArq.length - 80)}</span>}
                </div>
              </div>
            )}
          </div>
          ...
          {conflitos.length > 0 && (
            <div style={{ ...CARD, border: '1px solid rgba(239,68,68,.25)' }}>
                ...
                <tbody>
                  {conflitos.slice(0, 50).map((c, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,.03)' }}>
                      <td style={{ padding: '6px 10px', fontFamily: 'monospace', color: '#fbbf24', fontWeight: 700 }}>{c.matricula}</td>
                      <td style={{ padding: '6px 10px', fontFamily: 'monospace', color: '#94a3b8' }}>{fmtData(c.data)}</td>
                      <td style={{ padding: '6px 10px', fontFamily: 'monospace', fontSize: 11 }}>
                        {c.codigos.map((cod, j) => (
                          <span key={j} style={{ marginRight: 8, color: '#f87171' }}>{cod}</span>
                        ))}
                      </td>
                    </tr>
                  ))}
              ...
              {conflitos.length > 50 && (
                <div style={{ fontSize: 11, color: '#475569', padding: '8px 10px' }}>…e mais {fmt(conflitos.length - 50)} conflitos</div>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={() => setEtapa(2)} style={BTN('#6366f1')}>Revisar e importar →</button>
          </div>
        </>
      )}
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
            {[
              ...
              ['Secretarias',         `${fmt(join?.porSecretaria?.length ?? 0)} identificadas`, '#3b82f6'],
      ...
      {etapa === 3 && resultado && (
        <div style={CARD}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
            ...
            <KPI label="Sec. publicadas"      val={fmt(resultado.publicadas ?? 0)}    sub="em previas_publicadas"      cor="#3b82f6" />
          ...
          <div style={{ fontSize: 11, color: '#475569', marginBottom: 20 }}>
            Arquivo: <b style={{ color: '#94a3b8' }}>{resultado.arquivo}</b> · Tabelas: previas_frequencia, previas_publicadas
          </div>
          ...
          <button onClick={reiniciar} style={BTN('#6366f1')}>
